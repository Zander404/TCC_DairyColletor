...
async def start_collect_article_id(start: int = 0, limit: int = 1000, step: int = 1000):
    asyncio.create_task(core.collect_articleID(
...
async def start_collect_abstract(start: int = 0, limit: int = 0, max_threads: int = 10):
    asyncio.create_task(
        core.collect_abstract(start=start, limit=limit,
                              max_threads=max_threads)
    )
    # Retorna um CSV já existente
    return FileResponse(
        path="api/test.csv", media_type="text/csv", filename="resumos_em_progresso.csv"
    )
