...
class Scorer:
    ...
    def demo_convert(self, demos, template):
        ...
        for demo in demos:
            ...
            refhyp_demo = template.replace(
                "XXXXX", ref_line).replace("YYYYY", hyp_line)
            refhyp_demos.append(refhyp_demo)
            hypref_demo = template.replace(
                "XXXXX", hyp_line).replace("YYYYY", ref_line)
    ...
    def score(self, metrics):
        ...
        for metric_name in metrics:
            if metric_name in [
                ...
                opt_scorer = OPTScorer(
                    device=self.device, checkpoint=checkpoint)
                ...
                for e_asp in eval_asps:
                    ...
                    print('asp_df: ', asp_df)
                    refhyp_templates = [
                        "XXXXX In other words , YYYYY", " In other words , "]
                    template = refhyp_templates[0]  # template
                    refhyp_demos, hypref_demos = self.demo_convert(
                        demo, template)
                    ...
                    for doc_id in self.data:
                        ...
                        ref_summs = [add_dot(detokenize(line))
                                     for line in ref_summs]
                        sys_summ = add_dot(detokenize(
                            self.data[doc_id]['sys_summ']))
                        ...
                        ref_hypo_scores = np.array(opt_scorer.score(ref_summs1, [
                                                   sys_summ] * len(ref_summs), prompt_text=refhyp_templates[1], batch_size=1))
                        # hypo->ref
                        # define the prefix text...
                        if self.args.use_ist and self.args.use_demo:
                            hypref_demos_str = "\n".join(hypref_demos)
                        ...
                        hypo_ref_scores = np.array(opt_scorer.score(
                            sys_summ1, ref_summs, prompt_text=refhyp_templates[1], batch_size=1))

                        ref_hypo = ref_hypo_scores.max()
                        hypo_ref = hypo_ref_scores.max()
                        avg_f = (
                            0.5 * (ref_hypo_scores + hypo_ref_scores)).max()
                        harm_f = (ref_hypo_scores * hypo_ref_scores /
                                  (ref_hypo_scores + hypo_ref_scores)).max()
                        ...
                        if self.args.use_ist:
                            self.data[doc_id]['scores'][f'{metric_name}_{
                                e_asp}_ref_hypo'] = ref_hypo
                            self.data[doc_id]['scores'][f'{metric_name}_{
                                e_asp}_hypo_ref'] = hypo_ref
                            self.data[doc_id]['scores'][f'{
                                metric_name}_{e_asp}_avg_f'] = avg_f
                            self.data[doc_id]['scores'][f'{
                            ...
                            self.data[doc_id]['scores'][f'{
                                metric_name}_hypo_ref'] = hypo_ref
                            self.data[doc_id]['scores'][f'{
                                metric_name}_avg_f'] = avg_f
                            self.data[doc_id]['scores'][f'{
                                metric_name}_harm_f'] = harm_f
                print(f'Finished calculating OPTScore, time passed {
                      time.time() - start}s.')
                opt_scorer = None
                ...
                for e_asp in eval_asps:
                    ...
                    for doc_id in self.data:
                        ...
                        if self.args.use_ist and self.args.use_demo:
                            ...
                            prefix = ''
                        ref_summs1 = [
                            prefix + template.replace('XXXXX', line).replace('YYYYY', '') for line in ref_summs]
                        ...
                        sys_summ1 = [sys_summ] * len(ref_summs)
                        sys_summ1 = [
                            prefix + template.replace('XXXXX', line).replace('YYYYY', '') for line in sys_summ1]
                        hypo_ref_scores = np.array(flan_scorer.score(
                            sys_summ1, ref_summs, batch_size=1))
                        ...
                        if self.args.use_ist:
                            ...
                            self.data[doc_id]['scores'][f'{
                                metric_name}_harm_f'] = harm_f
                print(f'Finished calculating FLANScorer, time passed {
                      time.time() - start}s.')
                flan_scorer = None
                ...
                for samp_id, doc_id in enumerate(self.data):
                    ...
                    for k, ref_summ in enumerate(ref_summs):
                        ...
                        if ref_summ in keep_seen_refsumm_score:
                            ...
                            if self.args.use_ist and self.args.use_demo:
                                ...
                                prefix = ''
                            input1 = template.replace(
                                "XXXXX", ref_summ).replace("YYYYY", "")
                            input1 = prefix + input1
                            output1 = lower_check(sys_summ)
                            ref_hypo_score = gpt3score(
                                input1, output1, self.args.gpt3model, self.args.api_key)
                            ref_hypo_scores.append(ref_hypo_score)
                            ...
                            if self.args.use_ist and self.args.use_demo:
                                ...
                                prefix = ''
                            input2 = template.replace(
                                "XXXXX", sys_summ).replace("YYYYY", "")
                            input2 = prefix + input2
                            output2 = lower_check(ref_summ)
                            hypo_ref_score = gpt3score(
                                input2, output2, self.args.gpt3model, self.args.api_key)
                            hypo_ref_scores.append(hypo_ref_score)

                            keep_seen_refsumm_score[ref_summ] = [
                                ref_hypo_score, hypo_ref_score]
                    ...
                    if self.args.use_ist:
                        self.data[doc_id]['scores'][f'{metric_name}_{
                            self.eval_asp}_ref_hypo'] = ref_hypo
                        self.data[doc_id]['scores'][f'{metric_name}_{
                            self.eval_asp}_hypo_ref'] = hypo_ref
                        self.data[doc_id]['scores'][f'{metric_name}_{
                            self.eval_asp}_avg_f'] = avg_f
                        self.data[doc_id]['scores'][f'{metric_name}_{
                            self.eval_asp}_harm_f'] = harm_f
                    else:
                        self.data[doc_id]['scores'][f'{
                            metric_name}_ref_hypo'] = ref_hypo
                        self.data[doc_id]['scores'][f'{
                        ...
                        self.data[doc_id]['scores'][f'{
                            metric_name}_harm_f'] = harm_f
                print(f'Finished calculating gpt3_score, time passed {
                      time.time() - start}s.')
...
def main():
    ...
    args.output = out_dir + '/' + data_name + args.out_dir_name + \
        '_usedemo[' + str(args.use_demo) + ']' + '_useist[' + \
        str(args.use_ist) + ']_OptFlanGpt2_score.pkl'
...
if __name__ == "__main__":
    ...
    evaluator = Scorer(model_name="gpt-3.5-turbo")
    ...
    for model in models:
        ...
        if nome_base:
            ...
            pd.DataFrame(resultados).to_csv(
                f"./resultados/gptscore/{nome_base}.csv")
