    ...
    def score(self, metrics):
        ...
        for metric_name in metrics:
            if metric_name in [
                ...
                print(
                    f"Finished calculating OPTScore, time passed {
                        time.time() - start
                    }s."
                )
                opt_scorer = None
                ...
                print(
                    f"Finished calculating FLANScorer, time passed {
                        time.time() - start
                    }s."
                )
                flan_scorer = None
                ...
                for samp_id, doc_id in enumerate(self.data):
                    ...
                    for k, ref_summ in enumerate(ref_summs):
                        print()
                        print(
                            "aspect: %s; samp_id: %d; ref_summ_id/total_ref_summ: %d/%d"
                        ...
                        if ref_summ in keep_seen_refsumm_score:
                            ...
                            if self.args.use_ist and self.args.use_demo:
                                ...
                                prefix = ""
                            input1 = template.replace("XXXXX", ref_summ).replace(
                                "YYYYY", ""
                            )
                            ...
                            if self.args.use_ist and self.args.use_demo:
                                ...
                                prefix = ""
                            input2 = template.replace("XXXXX", sys_summ).replace(
                                "YYYYY", ""
                            )
                    ...
                    if self.args.use_ist:
                        self.data[doc_id]["scores"][
                            f"{metric_name}_{self.eval_asp}_ref_hypo"
                        ] = ref_hypo
                        self.data[doc_id]["scores"][
                            f"{metric_name}_{self.eval_asp}_hypo_ref"
                        ] = hypo_ref
                        self.data[doc_id]["scores"][
                            f"{metric_name}_{self.eval_asp}_avg_f"
                        ] = avg_f
                        self.data[doc_id]["scores"][
                            f"{metric_name}_{self.eval_asp}_harm_f"
                        ] = harm_f
                    else:
                        self.data[doc_id]["scores"][f"{metric_name}_ref_hypo"] = (
                            ref_hypo
                ...
                print(
                    f"Finished calculating gpt3_score, time passed {
                        time.time() - start
                    }s."
                )
